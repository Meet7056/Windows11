...
function Pictures({ data }: { data: MediaType[] }) {
	...
	return (
		<>
			<Head>
				<title>MEET - Pictures</title>
				<link
					rel="canonical"
					href="https://www.MEET.dev/explorer/pictures"
				/>
				...
				{/* OpenGraph */}
				<meta property="og:title" content="MEET - Pictures" />
				<meta
					property="og:url"
					content="https://www.MEET.dev/explorer/pictures"
				/>
				<meta
...
export async function getStaticProps() {
	...
	const data = res.resources.map((image: MediaType) => {
		return {
			...
			filename:
				image.public_id.replace('images/', '').length > 5
					? image.public_id.replace('images/', '').slice(0, 5)
